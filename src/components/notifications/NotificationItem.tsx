...
export function NotificationItem({
  ...
  const buildDestination = (link: string | null) => {
    if (!link) return null;

    if (notification.type === 'workflow_request' && !/[?&]tab=workflow/.test(link)) {
      return `${link}${link.includes('?') ? '&' : '?'}tab=workflow`;
    }
  ...
  const handleClick = () => {
    ...
    const destination = buildDestination(notification.link);
    if (destination) {
